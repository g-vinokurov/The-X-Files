...
class ReportLoader:
    ...
    def __parse_report(cls, report_dir: pathlib.Path) -> Report:
        report_id = str(report_dir.name)

        report_file_name = 'report.xml'
        report_file_path = report_dir / report_file_name
        if not report_file_path.exists():
            raise FileNotFoundError(f'{report_id}: File {report_file_name} not found, skipped!')
        ...
        report_name = cls.__parse_report_metadata_item(report_head, 'name', report_id)
